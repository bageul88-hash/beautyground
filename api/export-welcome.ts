...
// 수출 페이지 첫 개설(영문 소개 첫 저장) 시 브랜드 담당자에게 보내는 웰컴 메일.
// 호출: /brand/export 편집기가 "빈 pitch_en → 채움" 전환 저장에 성공했을 때 1회 POST.
// 인증: Authorization: Bearer <supabase access token> — 토큰의 사용자에 연결된 브랜드에게만 발송되므로
// 제3자가 임의 브랜드로 발송을 유발할 수 없다.
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!SERVICE_ROLE || !GMAIL_USER || !GMAIL_APP_PASSWORD) {
    return res.status(503).json({ message: '메일 발송 설정이 없습니다' })
  }
  ...
  // 이 사용자에 연결된 브랜드 찾기 — 판매 파트너 계정(partners.user_id) 또는 수출 전용 계정(export_contacts)
  let partner: { id: string; brand_name: string; export_pitch_en: string | null } | null = null
  const { data: p1 } = await supabase.from('partners').select('id,brand_name,export_pitch_en').eq('user_id', uid).maybeSingle()
  if (p1) partner = p1
  if (!partner) {
    const { data: ec } = await supabase.from('export_contacts').select('partner_id').eq('user_id', uid).maybeSingle()
    if (ec?.partner_id) {
      const { data: p2 } = await supabase.from('partners').select('id,brand_name,export_pitch_en').eq('id', ec.partner_id).maybeSingle()
      if (p2) partner = p2
    }
  }
  if (!partner) return res.status(404).json({ message: '연결된 브랜드가 없습니다' })
  ...
  try {
    ...
    console.error('[export-welcome] 발송 실패:', e)
