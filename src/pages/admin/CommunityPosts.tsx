...
import { COMMUNITY_CATEGORY_META, type CommunityCategory } from '../../lib/community'
...
const CATEGORY_FILTERS: { value: CategoryFilter; label: string }[] = [
  { value: 'all', label: '전체' },
  { value: 'work', label: COMMUNITY_CATEGORY_META.work.label },
  { value: 'life', label: COMMUNITY_CATEGORY_META.life.label },
]
...
const EMPTY_FORM = { category: 'work' as CommunityCategory, tags: '', title: '', excerpt: '', body: '', thumbnail_url: '' }
...
export default function AdminCommunityPosts() {
  ...
  const load = async () => {
    setLoading(true)
    const { data } = await supabase.from('community_posts').select('*').order('created_at', { ascending: false }).limit(500)
  ...
  return (
    <>
      <header className="h-[60px] bg-paper border-b border-rule flex items-center px-8 sticky top-0 z-20">
        <p className="text-[15px] font-semibold text-ink">브랜드 커뮤니티 콘텐츠 관리</p>
      </header>

      <main className="max-w-[1300px] p-8">
        <h1 className="text-[22px] font-bold text-ink mb-2">브랜드 커뮤니티 콘텐츠 관리</h1>
        <p className="text-[13px] text-ink-soft mb-6">
          /brand/community 에서 로그인한 브랜드 회원사가 열람하는 글을 관리합니다.
          "업무"는 브랜드 회원 전체 공개, "사는이야기"는 구독(관심등록)한 회원에게만 노출됩니다.
        </p>
        ...
        <div className="bg-paper rounded-md border border-rule p-5 mb-6">
          ...
          <div className="grid sm:grid-cols-2 gap-3 mb-3">
            <select
              value={form.category}
              onChange={(e) => setForm((f) => ({ ...f, category: e.target.value as CommunityCategory }))}
              className={inputCls}
            >
              {(Object.keys(COMMUNITY_CATEGORY_META) as CommunityCategory[]).map((cat) => (
                <option key={cat} value={cat}>{COMMUNITY_CATEGORY_META[cat].label}</option>
              ))}
            </select>
          ...
          {form.category === 'life' && (
            <input
              placeholder="태그 (쉼표로 구분 — 예: 음악, 영화, 여행)"
              value={form.tags}
                ...
                {visible.map((post) => {
                  ...
                  return (
                    <tr key={post.id} className="border-b border-rule last:border-b-0">
                      ...
                      <td className="px-4 py-3 text-ink-soft whitespace-nowrap">{COMMUNITY_CATEGORY_META[post.category].label}</td>
