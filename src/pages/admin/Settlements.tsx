...
import type { Partner, Settlement, PartnerSettlementRow } from '../../lib/types'
...
export default function AdminSettlements() {
  ...
  const [rows, setRows] = useState<PartnerSettlementRow[]>([])
  ...
  const loadRows = async () => {
    ...
    setRows((data ?? []) as PartnerSettlementRow[])
    setLoadingRows(false)
  }
  ...
  const handleMarkPaid = async (row: PartnerSettlementRow) => {
    setPayingId(row.id)
    setError('')
  ...
  return (
    <>
      <header className="h-[60px] bg-paper border-b border-rule flex items-center px-8 sticky top-0 z-20">
        <p className="text-[15px] font-semibold text-ink">파트너 정산 관리</p>
      </header>

      <main className="max-w-[1100px] p-8">
        <h1 className="text-[22px] font-bold text-ink mb-4">파트너 정산 관리</h1>
                  ...
                  return (
                    <tr key={row.id} className="border-b border-rule last:border-b-0">
                      ...
                      <td className="px-4 py-3">
                        <span className={`inline-flex items-center rounded-pill px-2.5 py-1 text-[12px] font-medium ${badge.bg} ${badge.text}`}>{badge.label}</span>
                      </td>
